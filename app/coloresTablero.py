...
def tablero_inicial(obj_tablero):
	dic_colores = {
		0:'violet',
		1:'',
		2:'orange',
	...
	#num = random.randrange(1,4)   //VARIABLE PARA QUE EL TABLERO SEA ALEATORIO
	obj_tablero.comenzar(2)

	tablero=[]
	for i in range(15):  ### tablero botones
		row=[]
		for j in range(15):
			#El color del tablero viene dado desde el obj_tablero, este devuelve el valor, y segun el valor se busca en dic_colores
			dic={'ocup':False,'valor':False,'color':dic_colores[obj_tablero.tablero[i][j].valor],'palabra':False,'letra':False,'puntosx':0}
			lista=[j]
			if j==3:
				lista.append('modulo')
			elif j==9:
